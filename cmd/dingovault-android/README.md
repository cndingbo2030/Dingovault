...
Release tags trigger `.github/workflows/release.yml`, which runs `gomobile bind` and Gradle **assembleRelease** / **bundleRelease**. The workflow uses `android-actions/setup-android` plus `gomobile` (there is no maintained `noproto/gomobile-action`; steps are inlined for reproducibility).
